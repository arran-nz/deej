...
func (ams *AudioMeterService) GetActiveAudioProcesses() (map[string]bool, error) {
	...
	for deviceIdx := uint32(0); deviceIdx < deviceCount; deviceIdx++ {
		...
		ams.queryDeviceSessions(endpoint, activeProcesses)
		endpoint.Release()
	}

	// Log active processes at Info level (only when there are some)
	if len(activeProcesses) > 0 {
		ams.logger.Debugw("Active audio processes", "processes", activeProcesses)
	}
